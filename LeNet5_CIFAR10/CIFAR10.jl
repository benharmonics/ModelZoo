...
function load_data(args)
	...
	trainloader = DataLoader((xtrain, ytrain_oh),
		batchsize=args.batchsize, shuffle=true)
	testloader = DataLoader((xtest, ytest_oh), batchsize=args.batchsize)
...
function eval_loss_accuracy(loader, model, device)
	...
	for (x′, y′) ∈ loader
		...
		ŷ = model(x)
		ltot += loss(ŷ, y) * size(x)[end]
		acc += sum(onecold(ŷ |> cpu) .== onecold(y |> cpu))
		ntot += size(x)[end]
	end
	loss_val = round(ltot/ntot, digits=4)
	acc_val = round(100acc/ntot, digits=4)
...
function train(; kws...)
	...
	# Using GPU via CUDA if possible
	if use_cuda
	...
	@info "CIFAR-10: $(trainloader.nobs) train and $(testloader.nobs) test examples."
	
	model = device(LeNet5())
	@info "LeNet5 model: $(sum(length, params(model))) trainable parameters."
	...
	for epoch ∈ 1:args.epochs
		@showprogress for (x′, y′) ∈ trainloader
			x, y = device(x′), device(y′)
			gs = gradient(ps) do
				ŷ = model(x)
				loss(ŷ, y)

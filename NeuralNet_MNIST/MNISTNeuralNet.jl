...
# ╔═╡ c99d0490-2d7f-11ec-344c-1be1983a5fd0
using Flux, MLDatasets, PlutoUI

# ╔═╡ 0b0c3ffd-2ec4-4204-93ec-ce67c06ec6f5
using Flux: DataLoader, onehotbatch, onecold, crossentropy, @epochs

# ╔═╡ 4ffb0f01-cfd2-401e-a5e9-bde79ceaaf67
using Images: Gray

# ╔═╡ d5200c86-baab-4782-a78c-f697e132e979
using StatsBase: mean

# ╔═╡ f0b48f11-73b5-4d89-bbfc-4db3e8f89ec5
md"# Basic Neural Networks

Handwritten digit recognition solved with deep learning methods."

# ╔═╡ 76128e02-1c57-4d6f-96e0-ce3e24b5e7ff
md"```get_data``` downloads and manipulates our data, flattening the images into 1-D vectors and encoding the labels with the 'onehot' schema:"

# ╔═╡ bcf19c42-1ffe-49fd-a567-31b64723b8e7
function get_data()
	...
	# Flattening 28×28 image matrices into 784×1 vectors
	xtrain_flat = flatten(xtrain)
	xtest_flat = flatten(xtest)
	# Encoding our labels
	ytrain_oh = onehotbatch(ytrain, sort(unique(ytrain)))
	ytest_oh = onehotbatch(ytest, sort(unique(ytest)))
	
	xtrain_flat, ytrain_oh, xtest_flat, ytest_oh
end

# ╔═╡ cad1aa97-e26d-4518-8058-33c7337f31a0
md"What does our data look like, by the way?

Each image is a grayscale 28×28 picture. There are 60,000 in the training dataset and 10,000 in the test dataset."

# ╔═╡ f6ab3cd2-b94c-4832-bf0a-61c6f2a2ada0
(x, y) = MNIST.testdata(Float32); # We'll use these to view the images

# ╔═╡ 8caa415b-a106-40d9-af77-ec8afcd90ada
@bind n Slider(1:10000)

# ╔═╡ 637c9a70-2313-4aea-bfff-2f68e3aecf60
let img = reverse(x[:,:,n], dims=2) |> rotl90
	Gray.(img), y[n]
end

# ╔═╡ e10de20d-acf3-4be1-b41d-d67ac1c736bf
md"Basically any reasonable model is able to solve this problem.

We're going to just use two ```Dense``` (fully connected) layers followed by ```softmax``` to give us nice probabilities."

# ╔═╡ dbe20aea-101d-4ba0-95c3-83ff72b0ef33
model = Chain(
	Dense(28*28, 60, relu),
	Dense(60, 10),
	softmax
...
# ╔═╡ 60f6f034-c378-449a-a881-ab58fefa2644
md"Loss and optimization functions:"

# ╔═╡ a048c41c-9c48-44b4-902f-8cc2b7d26004
loss(x, y) = crossentropy(model(x), y)

# ╔═╡ 177156b2-df9f-4cd8-86fb-067cc475a201
opt = ADAM(0.05)

# ╔═╡ 5237dd9d-2e95-4c56-8382-552fbf2b59af
md"**Training**"

# ╔═╡ 833a0890-33a7-459d-99ce-0bababc173eb
xtrain, ytrain, xtest, ytest = get_data();

# ╔═╡ c8039041-36b3-4eb0-92a7-660273c7b9ad
@epochs 100 Flux.train!(loss, params(model), [(xtrain, ytrain)], opt)

# ╔═╡ 6a1a6daa-e80c-4552-9d63-43c79da36f66
md"Checking our model against the ground truth:"

# ╔═╡ 812c6556-e320-440b-8c3e-607452c38d0e
accuracy(x, y) = mean(model(x) |> onecold .== y |> onecold)

# ╔═╡ 3db4b245-cacc-4678-b9da-3f4e1f88b4c1
accuracy(xtest, ytest)

# ╔═╡ 354c40c5-6c2b-4ee2-9d60-1ff31fc86d04
md"Not bad!"

# ╔═╡ 383376e0-3c68-4eb5-9b4d-ecb672b4fcf3
@bind n′ Slider(1:10000)

# ╔═╡ 80b22d11-6e75-4c31-aafc-3925d7657e9f
let img = reverse(x[:,:,n′], dims=2) |> rotl90
	Gray.(img), onecold(model(xtest[:,n′]), 0:9)
end

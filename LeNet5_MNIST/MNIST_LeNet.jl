...
function load_data(args)
	# Downloading from MLDatasets
	ENV["DATADEPS_ALWAYS_ACCEPT"] = "true"
	
	xtrain, ytrain = MLDatasets.MNIST.traindata(Float32)
	xtest, ytest = MLDatasets.MNIST.testdata(Float32)
	
	# Reshaping images to have a color channel
	xtrain′ = reshape(xtrain, 28, 28, 1, :)
	xtest′ = reshape(xtest, 28, 28, 1, :)
	
	# Saving labels as onehotbatches
...
function train(; kws...)
	...
	@info "MNIST: $(trainloader.nobs) train and $(testloader.nobs) test examples"
	
	model = device(LeNet5())
	@info "LeNet5 model: $(sum(length, params(model))) trainable params"
	
	parameters = params(model)
	
	# Optimizer settings
	if args.λ > 0
		optimizer = Optimiser(WeightDecay(args.λ), ADAM(args.η))
	else

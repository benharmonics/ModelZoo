...
# ╔═╡ 7e036fd0-32fa-11ec-097e-5ba8c4d25e38
using Flux

# ╔═╡ 0fb11d82-2b5d-4f48-bdeb-996afff5b0cc
using Flux: onehotbatch, onecold, logitcrossentropy, normalise, @epochs

# ╔═╡ 66f566d2-1429-4a8e-a8d9-e8e337b38eb9
using StatsBase: sample, mean

# ╔═╡ ead72162-7fa7-460d-afb2-90051847772f
md"# Iris classification with a neural net"

# ╔═╡ a85dcb10-6b45-49ce-8023-2d048adeef07
import BSON, MLDatasets, Dates

# ╔═╡ c74f40e1-1203-4981-a9c4-0da4e06079ec
md"Convenience struct for options"

# ╔═╡ 5cc74f4a-cc6b-4a61-a38e-41b2cd766cbe
Base.@kwdef struct Args
	η = 1e-3 	# learning rate
	epochs = 300
	savepath = "runs/NeuralNet/"
end

# ╔═╡ 2a77c952-4602-4c6f-a790-ee8d15678be5
md"Downloading & parsing data"

# ╔═╡ cb766b06-bcbd-4f2b-b000-5a60c30d9090
function getdata()
	...
	x_norm = normalise(x)
	y_oh = onehotbatch(y, sort(unique(y)))
	
	itrain = sample(1:length(y), length(y)*3÷4; replace=false)
	itest = setdiff(1:length(y), itrain)
	
	xtrain, ytrain = Float32.(x_norm[:, itrain]), y_oh[:, itrain]
	xtest, ytest = Float32.(x_norm[:, itest]), y_oh[:, itest]
	
	xtrain, ytrain, xtest, ytest
end

# ╔═╡ 036af05a-77d8-4cfc-b478-2903cb14c515
md"Build & train a model"

# ╔═╡ 320b189d-1254-4757-8f7a-588b6b119a4d
function train(; kws...)
	args = Args(; kws...)
	
	xtrain, ytrain, xtest, ytest = getdata()
	
	m = Chain(Dense(4, 16, relu), Dense(16, 3), softmax)
	
	loss(x, y) = logitcrossentropy(m(x), y)
	
	opt = ADAM(args.η)
	
	@epochs args.epochs Flux.train!(loss, params(m), [(xtrain, ytrain)], opt)
	
	m, xtest, ytest
end

# ╔═╡ fe603991-d117-437c-82b9-144d41e3e289
md"**Let's try it out!**"

# ╔═╡ 9e7719d9-aebd-44d7-b069-36c1da50e9b5
m, xtest, ytest = train();

# ╔═╡ fec75b91-d621-44cf-ac09-6c8796acb76a
md"Accuracy score & confusion matrix"

# ╔═╡ 4366cd08-4315-48d5-bbd4-c30770a34426
accuracy(x, y) = mean(m(x) |> onecold .== y |> onecold)

# ╔═╡ 595caa6f-5794-4452-b7f2-9504a70f01d3
md"*Notation*: The transpose of matrix ```A``` is written ```transpose(A)``` or, equivalently, ```A'```."

# ╔═╡ a2c61197-811a-409e-b157-3dccd912dfe6
confusion(x, y) = (ŷ = onehotbatch(onecold(m(x)), 1:3); y * ŷ')

# ╔═╡ 9c3b9d26-3325-4043-81ea-c3a3bbddd02c
accuracy(xtest, ytest)

# ╔═╡ 845685b9-b16b-4f07-a4cc-efc6e22d2fab
confusion(xtest, ytest)

# ╔═╡ e7f8655c-1d55-4ec3-88d9-5f3353c856c0
md"Saving the model"

# ╔═╡ e1b3cedc-eb29-4c62-a0cc-c6d2141462f9
function save(model; kws...)
	args = Args(; kws...)
	!ispath(args.savepath) && mkpath(args.savepath)
	modelpath = joinpath(args.savepath, "model-$(Dates.today()).bson")
	
	BSON.@save modelpath model
end

# ╔═╡ e444fba4-35f6-48f2-b738-3dbaa621b72c
accuracy(xtest, ytest) > .92 && save(m)
